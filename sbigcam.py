...
240619-MS:  eliminated Windows option for SBIG driver because Python 12 no longer 
            likes escape sequence in literal ! 
...
class SBIGCam(object):
    ...
    def __init__(self, verbose=False):
        ...
        self.WIDTH = c_ushort(0)
        self.HEIGHT = c_ushort(0)
        # Include sbigudrv.so
        # if system() == 'Linux':
    ...
    def get_camera_info(self, request=0):
        ...
        if request in [0, 1]:
            ...
            info['cameraType'] = str(gcir.cameraType) if gcir.cameraType not in CAMERA_TYPE else CAMERA_TYPE[
                gcir.cameraType]
    ...
    def start_exposure(self):
        ...
        for i in range(self.HEIGHT.value):
            Error = self.SBIG.SBIGUnivDrvCommand(CC_READOUT_LINE, byref(rlp),
                                                 byref(cameraData, i * self.WIDTH.value * 2))  # the 2 is essential
            if Error != CE_NO_ERROR:
    ...
    def move_cfw(self, position):
        ...
        if not (isinstance(position, int)):
            if self.verbose:
                print(
                    'Invalid position, must be an int usually less than the number of the model (IE FW8 goes from 0-7).')
            return False
