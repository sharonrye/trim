...
#def window(img, x0, y0, x_size, y_size):
...
def multiCens(img, n_centroids_to_keep=2, verbose=False, write_fits=False, no_otsu=True, save_dir='', size_fitbox=10, image_threshold=1000.):
...
#    centers = center_of_mass(labeled, labels=labeled, index=range(1, nr_objects+1))
	...
	for i,x in enumerate(centers):
		x=x[0]
		if not isNaN(x[1]) and not isNaN(x[0]): 
			px=int(round(x[1]))
			py=int(round(x[0]))     
			data = img[py-nbox:py+nbox,px-nbox:px+nbox]
			params = fitgaussian(data)
